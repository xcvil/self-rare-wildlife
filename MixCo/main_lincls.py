...
parser.add_argument('--finetune-model', default=None, type=str,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.finetune_model:
        if os.path.isfile(args.finetune_model):
            print("=> loading checkpoint '{}'".format(args.finetune_model))
            ...
            print("=> loaded pre-trained model '{}' in epoch {}".format(args.finetune_model, epoch))
        else:
            print("=> no checkpoint found at '{}'".format(args.finetune_model))
    ...
    if args.evaluate:
        validate(val_loader, model, criterion, args, val_class_id)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        acc1, prec, recall, f1 = validate(val_loader, model, criterion, args, val_class_id)
...
def validate(val_loader, model, criterion, args, class_id):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
...
class ProgressMeter(object):
    ...
    def display(self, batch, args):
        prefix = ' lr: {:.4f}\t'.format(args.current_lr) + self.prefix
...
def f1_score(output, target, class_id):
    """Stupid way to calculate binary classification recall and precision"""
    with torch.no_grad():
